...
`src/tabpfn_classifier_pipeline/pipeline.py` verbatim, the 4-file snapshot manifest of the pinned
`Prior-Labs/tabpfn_3` checkpoint (`tabpfn-v3-classifier-v3_default.ckpt` at immutable revision
...
|---|---|---|
| `tabpfn_classifier_colab.ipynb` | `TASK-INFERENCE` | synthetic or BYOD table → `validate_inputs` → **in-context** fit on the pinned checkpoint (no gradient update) → `accuracy`, `balancedAccuracy`, `logLoss`, `rocAuc` vs the majority-class baseline (`majority_class_baseline`) → `evaluation_report` → artifact bundle → fresh-boundary reload (recomputed validation accuracy equal to the recorded value within `1e-6`) → new-row scoring → exports |
| `tabpfn_classifier_artifact_inference_colab.ipynb` | `ARTIFACT-INFERENCE` | externally produced bundle → `validate_artifact_bundle` before deserialisation (bundled `model.ckpt` must equal the pinned checkpoint) → `from_artifact` → `validate_new_rows` → predict → exports (`not-measurable` evaluation report) |

**The private-worker fine-tuning path is not carried.** `tabpfn-classifier-finetuner` is private and the TabPFN-3
...
```python
from tabpfn_classifier_pipeline import TabPFNClassifierPipeline, build_synthetic_dataset, read_dataset_zip, validate_inputs, evaluation_report
pipe = TabPFNClassifierPipeline.from_pretrained(weights_dir="weights/tabpfn-3-classifier")   # digest-verified checkpoint
...
fresh = TabPFNClassifierPipeline.from_artifact("artifact")            # validates the bundle, then reloads
```
...
| Standalone-notebook pin | `Prior-Labs/tabpfn_3` @ `24a16a89d245878b846555110985634aa2e656d7`, `tabpfn-v3-classifier-v3_default.ckpt` SHA-256 `d0d865d54dfbc524f5703104be90620182dca7e5fb2c16de72e9959ea18f3988` (`weights/tabpfn-3-classifier/dimer-base-manifest.json`, `docs/WEIGHTS.md`) |
